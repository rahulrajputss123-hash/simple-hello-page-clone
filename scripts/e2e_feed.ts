import {
  assembleFeaturedImpl,
  getFeaturedFeedImpl,
  refreshAllFeedsImpl,
} from "@/lib/offers/feed-cache.server";
...
async function main() {
  ...
  line("assemble US home (slots)", {
    count: us.length,
    offers: us.map((o) => ({ t: o.title, r: o.reward_amount, src: o.source })),
  });
  ...
  line("network ranking desc?", {
    rewards,
    isDescending: JSON.stringify(rewards) === JSON.stringify(sorted),
  });
  ...
  line("null country -> default", {
    country: nullCountry.country,
    count: nullCountry.offers.length,
  });
